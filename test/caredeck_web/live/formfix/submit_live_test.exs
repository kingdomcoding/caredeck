defmodule CaredeckWeb.Aid.SubmitLiveTest do
  ...
  test "submit button disabled when application is still :draft", ctx do
    ...
    {:ok, _view, html} = live(conn, ~p"/aid/#{ctx.application.id}/submit")
  ...
  test "submit transitions to :submitted when application is :ready_to_submit", ctx do
    ...
    Caredeck.Aid.ApplicationSection
    ...
    app = Ash.get!(Aid.Application, ctx.application.id, tenant: ctx.facility.id, authorize?: false)
    :ok = Aid.Applications.recompute_status(app)

    conn = sign_in_team(ctx.conn, ctx.care_team)
    {:ok, view, _html} = live(conn, ~p"/aid/#{ctx.application.id}/submit")

    view |> element("button[phx-click=submit]") |> render_click()

    updated = Ash.get!(Aid.Application, ctx.application.id, tenant: ctx.facility.id, authorize?: false)
